...
一个小型桌面工具：把一组网页链接批量导出并合并成一个 PDF，也可以把本地 PDF 转换为 DOCX。
...
在 `网页转 PDF` 页签里，把链接粘贴到输入框，选择输出 PDF 路径，然后点击 `生成 PDF`。

在 `PDF 转 DOCX` 页签里，选择输入 PDF 和输出 DOCX 路径，然后点击 `转换为 DOCX`。
...
```bash
python cli.py input.pdf --pdf-to-docx -o output.docx
```
...
- 如果目标 DOCX 正被 Word 打开，工具会自动写入带时间戳的新文件名。
- PDF 转 DOCX 会尽量保留原 PDF 的版式；扫描件 PDF 需要 OCR 后才适合转换为可编辑文本。
